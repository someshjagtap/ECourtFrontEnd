...
export class TokenInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<unknown>, next: HttpHandler): Observable<HttpEvent<unknown>> {
    ...
    if (this.token.valid() && this.shouldAppendToken(request.url) && !this.includeMobaUrl(request.url)) {
      return next
        .handle(
          request.clone({
  ...
  private shouldAppendToken(url: string) {
    return this.hasHttpScheme(url) || this.includeBaseUrl(url) || this.includeMobaUrl(url);
  }
  ...
  private includeMobaUrl(url: string) {

    const baseUrl = environment.mobaUrl.replace(/\/$/, '');
