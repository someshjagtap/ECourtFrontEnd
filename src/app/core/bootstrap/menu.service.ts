...
export class MenuService {
  ...
  addNamespace(menu: Menu[] | MenuChildrenItem[], namespace: string) {
    menu.forEach(menuItem => {
      ...
      if (menuItem.authority != null) {
      }
    });
  }

  /** Add namespace for translation. */
  handleAuthority(user,menu: Menu[] | MenuChildrenItem[]) {
    menu.forEach(menuItem => {
      if (menuItem.authority != null) {
        if (!this.hasAnyAuthority(user,menuItem.authority)) {
          menuItem.hide = true;
        } else {
          menuItem.hide = false;
          if (menuItem.children && menuItem.children.length > 0) {
            this.handleAuthority(user,menuItem.children);
          }
        }
      } else {
        if (menuItem.children && menuItem.children.length > 0) {
  ...
  hasAnyAuthority(user,requestedAuthorities: string[]): boolean {
    if (!user.authorities) {
      return false;
    }
